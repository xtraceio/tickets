...
- evidence scope mismatch 사유: 선택 row IP와 대표 시나리오 evidence IP가 서로 다릅니다. selectedRowIp=78.153.140.40, representativeScenarioIp=185.191.171.10
- 확인 대상 IP: 수동 확인 필요
- IP 근거 불일치: true
...
- representativeScenarioIp: 185.191.171.10
- representativeGroupIp: 185.191.171.10
- scenarioEvidenceIps: 185.191.171.10
- evidenceTotalEvents: 1
- evidenceDistinctAttackTypes: 2
- evidenceUniquePaths: 1
- evidenceSamplePaths: /ko/qna/q22-sase/
- 사용자 수동 확인 대기: true
- 브라우저 유지: false
- 자동 IP 차단 수행: false
- 확인 대상 IP: 수동 확인 필요
...
- 판단 이유: 동일 IP의 단일 요청에서 복수 공격 유형 또는 복수 위험도 신호가 함께 확인되었으나, 동일 IP 이벤트 수가 1건이고 고유 경로가 1개라 운영형 WAF-G005 승격 조건에는 도달하지 않았습니다.
...
- representativeReason: 동일 IP 185.191.171.10에서 복수 공격 유형 또는 복수 위험도 신호가 함께 확인되었습니다.
...
flowchart TD
  ...
  nodeRepresentativeIp["대표 시나리오 근거 IP: 185.191.171.10"]
  nodeFinalIp["최종 차단 대상 IP: 수동 확인 필요"]
  nodeWAFS010Ip["WAF-S010 근거 IP: 185.191.171.10"]
...
| WAF-S003 Scanning Middle threshold | 40 | false | 85.208.96.204 (다른 IP 기반 미매칭 근거) | 스캐닝성 MIDDLE 이벤트가 기준 20건에 도달하지 않았습니다. |
| WAF-S004 Scanning High threshold | 50 | false | 185.191.171.10 (다른 IP 기반 미매칭 근거) | 스캐닝성 HIGH 이벤트가 기준 10건에 도달하지 않았습니다. |
| WAF-S005 Scanning Critical threshold | 65 | false | 185.191.171.10 (다른 IP 기반 미매칭 근거) | 스캐닝성 CRITICAL 이벤트가 기준 3건에 도달하지 않았습니다. |
...
| WAF-S008 AI malicious probability | 75 | false | 85.208.96.204 (다른 IP 기반 미매칭 근거) | AI 분석 결과가 없어 수동차단 후보로 올리지 않습니다. |
| WAF-S009 Threat sequence judgment | 80 | false | 85.208.96.204 (다른 IP 기반 미매칭 근거) | 로그 순서 기반 위협 시퀀스가 확인되지 않았습니다. |
| WAF-S010 Mixed attack types by same IP | 60 | true | 185.191.171.10 | 동일 IP 185.191.171.10에서 복수 공격 유형 또는 복수 위험도 신호가 함께 확인되었습니다. |
| WAF-S011 Distributed similar attack burst | 85 | false | 185.191.171.10 (다른 IP 기반 미매칭 근거) | 분산 유사 공격 기준에 도달하지 않았습니다. |

## 시나리오 상세 근거

### WAF-S010 Mixed attack types by same IP
- priorityScore: 60
- attackerIp: 185.191.171.10
- attackerIps: -
- reason: 동일 IP 185.191.171.10에서 복수 공격 유형 또는 복수 위험도 신호가 함께 확인되었습니다.
- recommendation: 웹 화면에서 해당 IP의 요청 경로와 탐지/차단 이력을 확인한 뒤 IP주소 수동차단을 검토하세요.
- evidence: {"threshold":2,"totalEvents":1,"distinctAttackTypes":2,"attackTypes":["LFI","SCANNER"],"severities":["CRITICAL","HIGH","MIDDLE"],"uniquePaths":1,"samplePaths":["/ko/qna/q22-sase/"]}


## 최종 IP 차단 검토 사유

최종 IP 차단 검토 사유: WAF-S010 동일 IP 185.191.171.10에서 복수 공격 유형 또는 복수 위험도 신호가 함께 확인되었습니다. 자동 IP 차단은 수행하지 않으며 담당자가 웹 화면에서 수동 확인 후 결정합니다.
