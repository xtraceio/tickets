...
대표 운영형 시나리오: WAF-G005 복합 공격 유형 검토. 공격자 IP 172.212.217.10에서 오늘 총 20개 정탐이 탐지되었습니다. 동일 IP에서 복수 공격 유형과 다수 경로가 함께 확인되어 IP주소 수동차단 검토가 필요합니다.

- 대표 운영형 시나리오: WAF-G005 복합 공격 유형 검토
- 운영형 판단 사유: 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다.
- 상세 근거: WAF-S003, WAF-S010
...
- 판단 이유: 대표 운영형 시나리오 WAF-G005 복합 공격 유형 검토 매칭: 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다.
...
- matchedScenarios: [3, 10]
- matchedCount: 2
- representativeScenario: WAF-S010 Mixed attack types by same IP
- representativePriorityScore: 60
- representativeReason: 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다.
...
- representativeGroupReason: 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다.
- representativeLowLevelScenario: WAF-S010

| group | priorityScore | matched | mapped | matchedLowLevel | reason |
|---|---:|---:|---|---|---|
| WAF-G001 반복 스캐닝 기반 IP 차단 검토 | 50 | true | 2, 3, 4, 5 | 3 | 동일 IP 172.212.217.10에서 스캐닝성 MIDDLE 이벤트가 20건 확인되어 기준 20건 이상입니다. |
...
| WAF-G005 복합 공격 유형 검토 | 80 | true | 10 | 10 | 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다. |
...
| WAF-S005 Scanning Critical threshold | 65 | false | 172.212.217.10 | 스캐닝성 CRITICAL 이벤트가 기준 3건에 도달하지 않았습니다. |
...
| WAF-S010 Mixed attack types by same IP | 60 | true | 172.212.217.10 | 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다. |
...
- evidence: {"severity":"MIDDLE","threshold":20,"count":20,"uniquePaths":20,"samplePaths":["/yindu.php","/debug-php.php","/conf.php","/php-fpm.php","/.env.test.php","/oauth.php","/u.php","/000.php"]}

### WAF-S010 Mixed attack types by same IP
- priorityScore: 60
- attackerIp: 172.212.217.10
- attackerIps: -
- reason: 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다.
- recommendation: 웹 화면에서 해당 IP의 요청 경로와 탐지/차단 이력을 확인한 뒤 IP주소 수동차단을 검토하세요.
- evidence: {"threshold":2,"totalEvents":20,"distinctAttackTypes":2,"attackTypes":["SCANNER","LFI"],"severities":["MIDDLE","CRITICAL","HIGH"],"uniquePaths":20,"samplePaths":["/yindu.php","/debug-php.php","/conf.php","/php-fpm.php","/.env.test.php","/oauth.php","/u.php","/000.php"]}


## 최종 IP 차단 검토 사유

최종 IP 차단 검토 사유: WAF-S003 동일 IP 172.212.217.10에서 스캐닝성 MIDDLE 이벤트가 20건 확인되어 기준 20건 이상입니다. / WAF-S010 동일 IP 172.212.217.10에서 복수 공격 유형 또는 복수 위험도와 다수 경로가 함께 확인되었습니다. 자동 IP 차단은 수행하지 않으며 담당자가 웹 화면에서 수동 확인 후 결정합니다.
...
- 요청 Method: GET
- URI/path: /.env.test.php
- 쿼리/요청본문: -
- HTTP 상태값: 406
- 탐지/차단 유형: 차단(OWASP)
- WAF 필터명: 환경 파일 접근UA 누락 + PHP 파일 요청제한 파일 접근
- 위험도/risk: SCANNER>CRITICALSCANNER>MIDDLELFI>HIGH
- 탐지 시각: 2026-05-01 11:14:18.010
- 분석 탭 수집: true
- 로그 탭 수집: true
